...
if(isset($_GET['mode']) && intval($_GET['id']) > 0)
{
	...
	$db->query("DELETE FROM `da_product` WHERE id = ".$id);
	$deletedata = $db->query("select *from product_images where producttid = ".$id); 
	if($deletedata->size()>0)
	{
	   while($rows = $deletedata->fetch())
	   {
	       $deleteid = $rows['id'];
	       $oldfile= $rows['file_name'];
	       $oldfile= "../post_images/".$oldfile;
		unlink($oldfile);
	       $db->query("DELETE FROM `product_images` WHERE id= ".$deleteid);
	   }
	}
	$_SESSION['msg']	=	'Successfully deleted';
	adheader('product-list.php');
}
$query	=	$db->query("SELECT * FROM `da_product` ORDER BY id DESC limit 50 ");
		...
		{$count++;
            ?>
                ...
                <td><?php echo $count;?></td>
                  ...
                  <td><a onclick="return confirmDelete();" href="<?php echo admin_path?>product-list.php?mode=delete&id=<?php echo $rows['id'];?>">Delete</a></td>
                </tr>
                <?php } 
			   } ?>
                </tbody>
