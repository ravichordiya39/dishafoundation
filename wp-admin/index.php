...
if(isset($_POST['btnsubmit']))
{
	...
	{
		$sql	=	"SELECT * FROM admin WHERE UPPER(username) = '".$username."' AND password = '".md5($password)."' ";
		$query	=	$db->query($sql);
		...
		if($query->size() <= 0)
		{
			...
			$rows	=	$query->fetch();
			$_SESSION[md5('apiadmin')]	=	$rows['id'];
			$_SESSION[md5('apiadminname')]	=	$rows['name'];
			adheader('dashboard.php');
		}
	}
}
?>
